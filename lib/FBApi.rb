...
module FBApi
    ...
    class Session
        ...
        def user
            ...
            @user = User.new(FBApi.call(@api_key, @secret_key, "Users.getInfo", "session_key"=>@session_key, "uids"=>@uid, "fields"=>User.all_fields)[0], self)
            @user
        end
        ...
        def call(method, params={})
            return FBApi.call(api_key, secret_key, method, params.update("session_key"=>session_key))
        end
...
#            puts 'merge_aid=' + ret.inspect
    ...
    # THIS IS FOR THE OLD FACEBOOK API, NOT THE GRAPH ONE. See FBApi.get and FBApi.post for Graph API
    #
    ...
    def FBApi.call(api_key, secret, method, kwargs)
        ...
        if fb_method == "photos.upload"
            ...
            response = FBApi.post_upload(file_name, kwargs)
        else
    ...
    def FBApi.post_upload(filename, kwargs)
    ...
    def FBApi.verify_signature(secret, arguments)
    ...
    def FBApi.parse_cookie_information(app_id, cookies)
    ...
    # * secret - the connect application secret
    # * cookies - the cookies given by facebook - it is ok to just pass all of the cookies, the method will do the filtering for you.
    def FBApi.verify_cookie_signature(app_id, secret, cookies)
        fb_keys = FBApi.parse_cookie_information(app_id, cookies)
    ...
    def FBApi.verify_connect_signature(api_key, secret, cookies)
        ...
        FBApi.verify_cookie_signature(api_key, secret, cookies)
    end
    ...
    class OAuthSession
        ...
        def get(id, options={})
            FBApi.get(@access_token, id, session_options(options))
        end

        def post(id, options={})
            FBApi.post(@access_token, id, session_options(options))
        end

        def fql(fql_query, options={})
            FBApi.fql(@access_token, fql_query, session_options(options))
        end

        def multifql(fql_queries, options={})
            FBApi.multifql(@access_token, fql_queries, session_options(options))
        end

        def rest(api_method, options={})
            FBApi.rest(@access_token, api_method, session_options(options))
        end

        # Returns a GraphObject for the given id
        def graph_object(id)
            FBApi::GraphObject.new(self, id)
        end
    ...
    class GraphObject
        ...
        def initialize(session_or_token, id)
            @oauth_session = if session_or_token.is_a?(FBApi::OAuthSession)
                session_or_token
                             else
                                 FBApi::OAuthSession.new(session_or_token)
                             end
            @id = id
...
#      resp = RestClient.get url
    ...
    # Example:
    #
    # FBApi.multifql(access_token, { :statuses => "SELECT status_id, message FROM status WHERE uid = 12345",
    ...
    def self.fetch(url, options={})
        ...
        begin
            ...
            raise FBApi::FaceBookError.new(ex.http_code, "#{res_hash["error"]["type"]}: #{res_hash["error"]["message"]}")
        end
...
#
    ...
    def FBApi.validate(secret, arguments)
